def read_true_data(filename):
    ...
    for line in f:
        ...
        true_data.append([float(line_spl[0]),float(line_spl[1]), float(line_spl[2])])
...
def read_input_data(filename):
    ...
    for line in f:
        ...
        line_s  = line.split('\n')    
        line_spl  = line_s[0].split(' ')
        input_data[timestamp] = {'v':float(line_spl[0]), 'w':float(line_spl[2]), 'dt':float(line_spl[3])}
